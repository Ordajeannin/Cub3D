...
/*   Updated: 2024/03/12 16:17:23 by ajeannin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int get_ipx(t_player *p, int i_py, double angle)
{
	...
	res = 0;
	if (angle < 90.0)
		i_px2 = p->pos_x + ((p->pos_y - i_py) * round(tan(dtor(90.0 - angle))));
	else if (angle < 180.0)
		i_px2 = p->pos_x - ((p->pos_y - i_py) * round(tan(dtor(angle - 90.0))));
	else if (angle < 270.0)
		i_px2 = p->pos_x - ((i_py - p->pos_y) * round(tan(dtor(270.0 - angle))));
	else
		i_px2 = p->pos_x + ((i_py - p->pos_y) * round(tan(dtor(angle - 270.0))));
...
int get_ipy(t_player *p, int i_px, double angle)
{
	...
	res = 0;
	if (angle < 90.0)
		i_py2 = p->pos_y - ((i_px - p->pos_x) * round(tan(dtor(angle))));
	else if (angle < 180)
		i_py2 = p->pos_y - ((p->pos_x - i_px) * round(tan(dtor(180.0 - angle))));
	else if (angle < 270)
		i_py2 = p->pos_y + ((p->pos_x - i_px) * round(tan(dtor(angle - 180.0))));
	else
		i_py2 = p->pos_y + ((i_px - p->pos_x) * round(tan(dtor(360.0 - angle))));
